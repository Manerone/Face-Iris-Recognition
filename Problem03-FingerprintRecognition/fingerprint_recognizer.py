...
class FingerprintRecognizer:
    ...
    def _classify(self, image):
        image_enhanced = self._image_enhancement(image)
        blurred_image = signal.medfilt(image_enhanced, 5)

        # plt.imshow(blurred_image, cmap='Greys_r')
        # plt.show()

        orientations, averages_x, averages_y = self._orientation_computation(
            blurred_image
        )
        # self._show_orientation_lines(image, orientations)

        interesting_blocks = self._regions_of_interest(image_enhanced)
        # self._show_interesting_blocks(image, interesting_blocks)

        smoothed_orientations = self._smooth_orientations(
            averages_x, averages_y, interesting_blocks
        )
        # self._show_orientation_lines(image, smoothed_orientations)

        cores, deltas = self._singular_points_detection(
            smoothed_orientations, interesting_blocks
        )
        classification = self._classify_based_on(cores, deltas, interesting_blocks)
        return classification

    def _minutiaes(self, image):
        image_enhanced = self._image_enhancement(image)

        binarized_image = self._image_binarization(image_enhanced)

        # plt.imshow(binarized_image, cmap='Greys_r')
        # plt.show()

        smoothed_image = self._smooth_image(binarized_image)

        # plt.imshow(smoothed_image, cmap='Greys_r')
        # plt.show()

        thin_image = self._image_thinning(smoothed_image)

        # plt.imshow(thin_image, cmap='Greys_r')
        # plt.show()

        interesting_blocks = self._regions_of_interest(image_enhanced)
        minutiaes = self._minutiaes_detection(thin_image, interesting_blocks)
